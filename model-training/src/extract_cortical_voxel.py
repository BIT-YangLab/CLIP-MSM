...
def extract_cortical_mask(subj, roi="", output_dir="./output"):
    if roi != "":
        roi_tag = "_" + roi
    else:
        roi_tag = ""
...
def extract_voxels(
    ...
    output_path = (
        ...
        % (output_dir, zscore_tag, subj, tag)
    )
    ...
    if mask is None:
        try:
            mask = np.load(
                "%s/voxels_masks/subj%d/cortical_mask_subj%02d_%s.npy"
                % (output_dir, subj, subj, tag)
            )
        except FileNotFoundError:
            mask = extract_cortical_mask(subj, roi, output_dir)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("%s/cortical_voxels" % output_dir):
        os.makedirs("%s/cortical_voxels" % output_dir)
    ...
    for s in subj:
        if not os.path.isdir("%s/voxels_masks/subj%d" % (output_dir, s)):
            os.makedirs("%s/voxels_masks/subj%d" % (output_dir, s))
        if args.mask_only:
            print("Extracting ROI %s for subj%d" % (args.roi, s))
            extract_cortical_mask(s, roi=args.roi, output_dir=output_dir)
        else:
            extract_voxels(s, args.roi, args.zscore_by_run, output_dir=output_dir)
