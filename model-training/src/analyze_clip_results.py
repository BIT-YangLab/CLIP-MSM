...
def process_bootstrap(subj, model):
    rsq = np.load( "%s/output/bootstrap/subj%01d/rsq_dist_%s_whole_brain.npy"
        % (args.output_root, subj, model))
    fdr_p = fdr_correct_p(rsq)
    print("fdr_p:", fdr_p)
    import os
    os.makedirs("%s/output/ci_threshold" % (args.output_root), exist_ok=True)
    np.save(
        "%s/output/ci_threshold/%s_fdr_p_subj%01d.npy"
        % (args.output_root, model, subj),
        fdr_p,
    )
...
if __name__ == "__main__":
    ...
    if args.process_bootstrap_results:
        process_bootstrap(args.subj, "clip_visual_resnet") # subj, model
