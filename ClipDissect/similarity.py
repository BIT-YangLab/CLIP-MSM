...
def soft_wpmi(clip_feats, target_feats, top_k=100, a=1, lam=1, device='cuda',
                        min_prob=1e-7, p_start=0.998, p_end=0.97):
    
    with torch.no_grad():
        torch.cuda.empty_cache()
#         print("test clip 1: ",clip_feats)
        clip_feats = torch.nn.functional.softmax(a*clip_feats, dim=1)
#         clip_feats = torch.nn.functional.softmax(clip_feats, dim=1)
#         print("test clip 2: ",clip_feats)
...
#         print("inds[0]: ",inds[0])
        ...
        print("tqdm: ",range(target_feats.shape[1]))
...
#             print("inds[:,orig_id:orig_id+1]: ",inds[:,orig_id:orig_id+1])
