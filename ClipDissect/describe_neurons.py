...
current_model_id, current_subject, current_roi = torch.load(root_path + "/result/NSD_train_info.pt")
...
roi_list = ['ffa', 'eba', 'rsc', 'vwfa', 'food', 'full']
...
_concept_set = current_path + "/data/selective_concpet.txt" # dissecting labels
...
_result_dir = root_path + "/result/NSD_DissectResult/%sResult/%s_%s_%s_SELECTIVE_ROI_%s_Subj0%d"%(_target_model, _target_model,_target_layers,_d_probe, roi_list[current_roi], current_subject)
...
weigths_root_list = [
    root_path + "/model-training/output/encoding_results/clip_visual_resnet_SELECTIVE_ROI/subj{}/34_session/weights_clip_visual_resnet_SELECTIVE_ROI.npy", 
    root_path + "/model-training/output/encoding_results/clip_vit_SELECTIVE_ROI/subj{}/34_session/weights_clip_visual_resnet_SELECTIVE_ROI.npy", 
    root_path + "/model-training/output/encoding_results/convnet_resnet_SELECTIVE_ROI/subj{}/34_session/weights_clip_visual_resnet_SELECTIVE_ROI.npy", 
    root_path + "/model-training/output/encoding_results/convnet_alexnet_SELECTIVE_ROI/subj{}/34_session/weights_clip_visual_resnet_SELECTIVE_ROI.npy",
]
...
# load roi index
ffa=np.loadtxt(current_path + "/data/roi_index/subj0{}_FFA_index.txt".format(current_subject),dtype=np.float32,delimiter=",")
eba=np.loadtxt(current_path + "/data/roi_index/subj0{}_EBA_index.txt".format(current_subject),dtype=np.float32,delimiter=",")
rsc=np.loadtxt(current_path + "/data/roi_index/subj0{}_RSC_index.txt".format(current_subject),dtype=np.float32,delimiter=",")
vwfa=np.loadtxt(current_path + "/data/roi_index/subj0{}_VWFA_index.txt".format(current_subject),dtype=np.float32,delimiter=",")
food=np.loadtxt(current_path + "/data/roi_index/subj0{}_FOOD_index.txt".format(current_subject),dtype=np.float32,delimiter=",")
index=np.zeros(len(weights),dtype=bool)

# select region of interests
roi_info_list = [ffa, eba, rsc, vwfa, food, 0]
if current_roi < 5:
...
if __name__ == '__main__':
    ...
    for target_layer in args.target_layers:
        ...
        # hard dissect：Record the label with the highest score for each voxel similarity score
        top10_vals, top10_ids = torch.topk(similarities, k=5, dim=1)
        ...
        for top_neu_ids in top10_ids:
            ...
            top_des.append(des)
        # df_top10=pd.DataFrame(top_des,columns=['top1','top2','top3','top4','top5','top6','top7','top8','top9','top10'])

        df_top10=pd.DataFrame(top_des,columns=['top1','top2','top3','top4','top5'])
        ...
        # soft dissect：Perform softmax for each voxel on the similarity score of all labels
